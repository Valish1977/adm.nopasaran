...
export default class Auth {
        ...
        return new Promise((resolve) => {
            Vue.$axios.post("/api/rpc/login", {
                ...
                if (response.status === 200) {
                    ...
                    this.setUser(user);
                    ref.testVersions(response.data[0].ref_version);
                    router.push({ path: "/" + user.RoleCode + "/dashboard" });
                } else {
    ...
    public setUser(user: any): void {
        ...
        localStorage.setItem("user", JSON.stringify(user)); // а почему этим не стор вообще занимается?
