...
<template>
  <div>
    <el-dialog
      :visible="isVisible && type == 'edit' && editType == 'profile'"
      width="1000px"
      top="5vh"
      @close="closeForm()"
      :close-on-click-modal="false"
    >
      <span slot="title" class="dialog-title">{{$t("User.editUserTitle")}}</span>
      <el-alert
...
import { updateItem, searchDuplicateCard } from "@/api/user";
        ...
        .then(() => {
          updateItem()
            .then((data: any) => {
              ...
              let v = this.orgItems.find(
                (t: any) => t.id.toString() === this.myForm.org_id.toString()
              );
