                  ...
                  >
                    ...
                    >
                      <el-select
                        ...
                        @change="modify({name:'way_id', data: myForm.way_id})"
                      >
                        <el-option
...
import { insertItem, uploadPhoto } from "@/api/event";
...
export default class AddEvent extends Vue {
  ...
  private stepper: any = 0;
  private tplparam: any = {};
  ...
  private setDataForm(): void {
    ...
    this.theme = 0; // тема
    this.stepper = 0;
    this.myForm.tplparam = this.references.ref_event_way[0].tplparam;
    this.modify({ name: "tplparam", data: this.myForm.tplparam });
    this.themeList = [];
  ...
  private delReport(index): void {
    if (this.contentList[index].photo.length > 0) {
      this.delPhoto(this.contentList[index].unix);
    }
    ...
    for (const n in this.photoList) {
      if (this.photoList[n] !== undefined) {
        for (const i in this.contentList) {
          if (this.contentList[i].unix === this.photoList[n].unixContent) {
            this.contentList[i].photo[0] = Object.assign({}, this.photoList[n]);
          }
        }
      }
    }
  ...
  private handleRemovePhoto(file, fileList): void {
    this.delPhoto(file.unixContent);
  }
  private delPhoto(index): void {
  ...
  // отправка формы
  // вначале делаем валидацию формы и запускаем отправку фото
    ...
    this.selfForm.validate((valid: boolean) => {
      ...
      this.uploads()
        .then(() => {
          ...
          this.modify({ name: "approved", data: true });
          this.modify({ name: "econtent", data: content });
          this.modify({ name: "ematerials", data: this.materialList });
          this.modify({
            name: "user_id",
            data: this.$store.getters.getUser.id
          });
          this.modify({ name: "org_id", data: this.$store.getters.getUser.id });
        })
        .then(() => insertItem())
        .then((data: any) => {
          this.$notify({
          ...
          v = this.references.ref_geo.find(
            (t: any) => t.id === this.myForm.geo_id
          );
