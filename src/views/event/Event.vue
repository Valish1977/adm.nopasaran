    ...
    <el-row  :gutter="20" style="margin-bottom:15px" justify="end">
      <el-col :span="24">
        ...
        >
          ...
          <el-table-column prop="approved" :label="this.$t('Event.tableHeader.approved')">
            <template slot-scope="scope">
              <el-checkbox :value="scope.row.approved" @change="approvedFn(!scope.row.approved, scope.row.id)"></el-checkbox>
            </template>
          </el-table-column>
...
import { activate, approved } from "@/api/event";
...
export default class Event extends Vue {
  ...
  get tableItems() {
    ...
    const items: any = this.$store.getters["entities/event"]()
      .orderBy("id", "desc")
      .get();
  ...
  private approvedFn(val, id): any {
    approved(val, id)
      .then(() => {
        let messageText = this.$t("notify.approvedText") as string;
        if (!val) {
          messageText = this.$t("notify.approvedText1") as string;
        }
        this.$notify({
          title: this.$t("notify.success") as string,
          type: "success",
        ...
        this.update({ id, data: { approved: val } });
      })
      .catch((error: any) => {
        this.$notify({
          title: this.$t("notify.attention") as string,
